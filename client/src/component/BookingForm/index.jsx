...
const index = ({ onSelectionChange }) => {
    ...
    useEffect(() => {
        ...
        const hasSelection = movie.trim() !== "" || slot !== "" || Object.keys(seats).length > 0;
                ...
                <div className='grid grid-cols-1  p-1'>
                    ...
                    <div className='border-2 rounded-2xl border-amber-500 m-3 p-4  text-black font-semibold'>
                        ...
                        <div className='grid gap-2 sm:grid-cols-2 md:grid-cols-5'>
                            ...
                            {movies.map((item) => (
                                <div key={item._id}
                                    className={`border-2 rounded-xl m-3 p-5 hover:scale-105 cursor-pointer 
                                        ${movie === item.name ? 'bg-orange-700' : 'bgc'}`}
                    ...
                    <div className='border-2 rounded-2xl border-amber-500 m-3 p-4  text-black font-semibold'>
                        ...
                        <div className='grid m-3 p-4 gap-2 sm:grid-cols-2 md:grid-cols-5'>
                            {slots.map((slotValue) => (
                                <div
                                    key={slotValue}
                                    className={`border-2 rounded-xl m-3 p-6 hover:scale-105 cursor-pointer 
                                        ${slot === slotValue ? 'bg-orange-700' : 'bgc' }`}
                    ...
                    <div className='border-2 rounded-2xl border-amber-500 m-3 p-4  text-black font-semibold'>
                        ...
                        <div className='grid m-3 p-4 gap-2 sm:grid-cols-2 md:grid-cols-5'>
                            ...
                            {seat1.map(seat => (
                                <div key={seat}
                                    className={`border-2 rounded-xl p-3 m-2 hover:scale-105 flex justify-evenly gap-7 
                                    ${seats[seat] ? 'bg-orange-700' : 'bgc'}`}
